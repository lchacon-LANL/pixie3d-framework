...
c$$$!$OMP.    ,offset)
c$$$!$OMP DO
c$$$!$OMP.REDUCTION(+:pp,np3) !!v_tot,vx2,vt2)
          do ipc = 1, npc_int(ii,is)

            pp = pp + pr

            if(pp>1d0) then  !This is problematic with OpenMP
              offset = 1
              pp = pp-1d0
              np3 = np3+1
            else
            ...
            vt(1) = v_thx(is+offset)
            vt(2) = v_thy(is+offset)
            vt(3) = v_thz(is+offset)
            ...
            do ipx = 0,1
              ...
              do ipy = 0,1
                ...
                do ipz = 0,1
                  ...
                  spcs(is)%pcles(ipl)%v_n(ip_ng,1)
     .                 = signx*vt(1)*rx1(ip_ng) + v_0x(is+offset)
                  spcs(is)%pcles(ipl)%v_n(ip_ng,2)   
     .                 = signy*vt(2)*rx2(ip_ng) + v_0y(is+offset)
                  spcs(is)%pcles(ipl)%v_n(ip_ng,3)   
     .                 = signz*vt(3)*rx3(ip_ng) + v_0z(is+offset)
