...
c       * bcond: array containing boundary conditions per equation
...
c     For arbitrary initial guess (rr = b - Ax)
      ...
      if (rr0.lt.abstol) then
        if (out.ge.1) write(*,*)'Initial solution is exact in CG' 
        return
      endif

c CG solve

      if (stp_test.eq.0) rr0 = sqrt(sum(b*b))

      iter = 0
      if (out.ge.2) write (*,10) iter,rr0,rr0/rr0
...
cc        if (mag1.lt.tol) exit
        ...
        if (mag.lt.(abstol + tol*rr0)) then
          if (mag > tol*rr0 .and. mag < abstol .and. out.gt.1) then
...
c     For arbitrary initial guess (vv(:,1) = b - Ax)
      ...
      if (rold.lt.abstol) then
        ierr = -1
        if (iout.ge.1) write(*,*) 'Initial solution is exact in GMRES'
        call killgm
        return
      endif
...
c Calculate magnitude of initial residual
      ...
      if (stp_test.eq.0) rold = sqrt(sum(b*b))
...
c      Check convergence and restart outer loop if necessary
        ...
        if (ro.le.(abstol+eps1)) then
          if (ro > eps1 .and. ro < abstol .and. iout.gt.1 ) then
            write (*,*) 'Solution in GMRES is in round-off.'
            write (*,*) 'Aborting GMRES...'
          endif
          ierr = 0
          exit
        endif
